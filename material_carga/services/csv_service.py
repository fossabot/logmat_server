import pandas as pd
from material_carga.models import Conta, Material, Setor

# ArquivoEntrada Service


def update_database(csv_file):
    file = pd.read_csv(filepath_or_buffer=csv_file, sep=";", encoding="iso-8859-1")
    ...
    for i in range(0, len(file)):
        ...
        try:
            ...
            conta = Conta.objects.get_or_create(numero=conta_numero, nome=conta_nome)[0]
            material = Material(
                setor=setor,
                conta=conta,
                ...
                vl_atualizado=float(
                    0
                    if not isinstance(file["VL. ATUALIZ."][i], str)
                    else file["VL. ATUALIZ."][i].replace(",", ".")
                ),
                vl_liquido=float(
                    0
                    if not isinstance(file["VL. LIQUIDO"][i], str)
